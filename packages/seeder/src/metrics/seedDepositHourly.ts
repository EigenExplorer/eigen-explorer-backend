...
export async function seedDepositHourly() {
    const prismaClient = getPrismaClient()
	const depositHourlyList: Omit<prisma.MetricDepositHourly, 'id'>[] = []

	const firstTimestamp = await fetchLastSyncTime(timeSyncKey) * 1000
	const { hour: lastTimestamp } = await prismaClient.hourly_deposit_data.findFirst({
		select: { hour: true },
		orderBy: { hour: 'desc' }
	}).then(result => ({ 
		hour: result?.hour?.getTime() ?? 0 })) || { hour: 0 }

	// Bail early if there is no time diff to sync
	if (lastTimestamp - firstTimestamp <= 0) {
		console.log(`[In Sync] [Metrics] DepositHourly from: ${firstTimestamp} to: ${lastTimestamp}`)
		return
	}
	...
	await bulkUpdateDbTransactions(
		dbTransactions,
		`[Data] Deposit from: ${firstTimestamp} to: ${lastTimestamp} size: ${depositHourlyList.length}`
	)

	// Storing last synced block
	await saveLastSyncBlock(timeSyncKey, lastTimestamp)
}
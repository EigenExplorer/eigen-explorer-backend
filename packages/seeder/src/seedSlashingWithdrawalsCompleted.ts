...
const blockSyncKey = 'lastSyncedBlock_completedSlashingWithdrawals'
const blockSyncKeyLogs = 'lastSyncedBlock_queuedSlashingWithdrawals' // Latest sync is with WithdrawalQueued table due to foreign key constraint on withdrawalRoot
 ...
 * @param toBlock
 */
export async function seedCompletedSlashingWithdrawals(toBlock?: bigint, fromBlock?: bigint) {
	...
	await loopThroughBlocks(
		...
		async (fromBlock, toBlock) => {
			const logs = await prismaClient.eventLogs_SlashingWithdrawalCompleted.findMany({
				where: {

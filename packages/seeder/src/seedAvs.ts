...
export async function seedAvs(toBlock?: bigint, fromBlock?: bigint) {
    ...
    const avsList: Map<string, { metadata: EntityMetadata, createdAtBlock: string, updatedAtBlock: string }> = new Map()
            ...
            try {
                ...
                if (log.args.metadataURI && isValidMetadataUrl(log.args.metadataURI)) {
                    ...
                    if (avsMetadata) {
                        ...
                        if (existing) {
                            avsList.set(avsAddress, { metadata: avsMetadata, createdAtBlock: existing.createdAtBlock, updatedAtBlock: blockNumber })
                        } else {
                            avsList.set(avsAddress, { metadata: avsMetadata, createdAtBlock: blockNumber, updatedAtBlock: blockNumber })
                        }
                    } else {
    ...
    if (firstBlock === baseBlock) {
        ...
        for (const [address, { metadata, createdAtBlock, updatedAtBlock }] of avsList) {
            newAvs.push({
                address,

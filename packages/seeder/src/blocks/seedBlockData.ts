...
export async function seedBlockDataV2(toBlock?: bigint, fromBlock?: bigint) {
    const viemClient = getViemClient();
    const prismaClient = getPrismaClient();
    ...
    await loopThroughBlocks(
        ...
        async (fromBlock, toBlock) => {
            ...
            const blocks = await Promise.all(promises);
            const newBlockData: { number: bigint, timestamp: Date }[] = [];

            for (const block of blocks) {
                const timestamp = new Date(Number(block.timestamp) * 1000);
                newBlockData.push({ number: block.number, timestamp });
            }

            await bulkUpdateDbTransactionsV2(
                [
                    { table: 'Evm_BlockData', data: newBlockData }
                ],

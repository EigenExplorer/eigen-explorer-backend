...
export const AvsHistoricalCountSchema = HistoricalCountSchema.extend({
	...
	value: z.number().describe('The total number of AVS registered ').openapi({ example: 10 })
})
...
export const DepositHistoricalCountSchema = HistoricalCountSchema.extend({
	timestamp: z
		...
		.openapi({ example: '2024-04-11T08:31:11.000Z' }),
	value: z.number().describe('The total number of deposits made ').openapi({ example: 10 })
})
...
export const StakerHistoricalCountSchema = z.object({
	timestamp: z
		...
		.openapi({ example: '2024-04-11T08:31:11.000Z' }),
	value: z.number().describe('The total number of AVS stakers registered ').openapi({ example: 10 })
})
...
export const WithdrawalHistoricalCountSchema = HistoricalCountSchema.extend({
	timestamp: z
		...
		.openapi({ example: '2024-04-11T08:31:11.000Z' }),
	value: z.number().describe('The total number of withdrawals made').openapi({ example: 10 })
})

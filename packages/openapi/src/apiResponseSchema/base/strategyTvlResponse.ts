...
const StrategyEthTvl = z
	.number()
	.describe('The total value locked (TVL) in the strategy, denominated in ETH')
	.openapi({ example: 1000000 })

const StrategyName = z.string().describe('The name of the strategy').openapi({ example: 'cbETH' })
...
export const StrategyEthTvlSchema = z
	...
	.describe('The total value locked (TVL) in each restaking strategy, denominated in ETH')
	.openapi({
		example: {
			cbETH: 1000000,
			stETH: 2000000
